...
import com.github.fma.Utils.{APPLICATION_ID, FROM_TOPIC, GROUP_ID, TIMEOUT_MILLS, TO_TOPIC, TweetsTable, getBootstrapServers, getDB, getNatsPort}
...
object NatsKafkaSlickDemo {
     ...
     */
        ...
        Future {
            ...
            val tweet: Tweet = serialization.read

                        [Tweet](
                            record.value)

            logger.info(
              s"""Adding Tweet with offset ${record.offset} and partition ${record.

                        partition} and key ${record.key}...""".stripMargin)

            db.run(tweetsTable ++= List(tweet
                                .toTweetTuple)
                                    ) onComplete {
              case Success(_) =>
                logger.info(
                  s"""Successfull
                            ith offset ${record.offset} and partition $
                                {record.
                                    partition} and key ${record.key}""".stripMargin)
        ...
        |and partition $
            {record.partition}
                 and key ${record.key}""".stripMargin, exception)
            }
          }
        }
      } finally
      ...
      case Success(_) =>
        ...
        "Successfully processed all tweets from Kafka Consumer")
      case Failure(exception) =>
        logger.error(
        "Error processing
            eets from Kafka Consumer", exception)
    }

    // TODO:
        ly tweets.json in tests should be a symlink, but I don't think that'd
         with
            nix/Windows
    val tweetI
              m =
      getClass.getClassLoader.getResourceAsStream("tw
        json")
    val
            source: Buf
        Source
